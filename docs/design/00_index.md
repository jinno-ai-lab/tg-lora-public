...
1.  [00_index.md](00_index.md): ドキュメント地図と用語集（本ファイル）
2.  [01_algorithm_dataflow.md](01_algorithm_dataflow.md): 1サイクルの完全データフローと処理シーケンス
3.  [02_cost_accounting.md](02_cost_accounting.md): 実 backward パス数に基づく効率会計
4.  [03_gating_and_N_selection.md](03_gating_and_N_selection.md): コサイン類似度制限と投機ステップ $N$ の決定
5.  [04_acceptance_and_validation.md](04_acceptance_and_validation.md): 答え合わせ（二階層検証）と承認/拒否の判定論理
6.  [05_subspace_m9.md](05_subspace_m9.md): Prior-based Subspace Learning (M9) の詳細
7.  [linearity_guard.md](linearity_guard.md): Linearity Guard (アブレーション用フォールバック安全装置)
8.  [06_experiment_protocol.md](06_experiment_protocol.md): 本番実験 (Seed 42,43,44) の公平性比較プロトコル
9.  [07_design_decisions_log.md](07_design_decisions_log.md): アーキテクチャの発展史と過去の設計決定の履歴
10. [08_claims_and_evidence_map.md](08_claims_and_evidence_map.md): 論文の主張とそれを支える証拠の対応マッピング
11. [10_guard_experiment.md](10_guard_experiment.md): Guard実験 — r_Aによる選択的学習・出力側連続固定・二段停止プロトコル
...
*   **コード上の対応物**: [train_tg_lora.py:L1482](../../src/training/train_tg_lora.py#L1482) 内の `for _ in range(pilot_K):` の実更新ループ。
...
*   **コード上の対応物**: [src/tg_lora/extrapolator.py](../../src/tg_lora/extrapolator.py) の `apply_extrapolation`。
...
*   **コード上の対応物**: [train_tg_lora.py:L2750-2790](../../src/training/train_tg_lora.py#L2750-L2790) の `eval_loss(model, valid_quick_loader, ...)`。
