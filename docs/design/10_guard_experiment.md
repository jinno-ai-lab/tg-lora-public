...
**実測配線（2026-06 追加・TASK-0135）**: 上記の着地点は定数偽造でのみ検証されていたが、`level1_realization_record_from_measurements(baseline_backward, level1_backwards)` が Phase 67 in-vivo suite の `_BackwardCounter`（live autograd graph の後方走査回数）を record に橋渡しする。配線した CPU-proxy A/B を depth {2,3,4} の 3 再生産で走らせると、Level-1（freeze-only）は**全て ~0** に実現する——層の重み勾配を凍結しても activation gradient は層を走査し続けるため（`RealizedReduction` の設計通り）。したがって非薄 record でも `observed_reduction≈0` となり、ceiling は 0.0、Level-1 判定は**正直に FAIL を維持**する。判定経路そのものは実測で駆動され観察可能になったが、正直な ~0 ゆえ反転はしない（偽造拒否）。非零実現（grad-ckpt 併用下等）による FAIL→PASS 回復は、9B/CUDA 環境の別作業として同じ adapter を通じて供給される。
...
**実測配線（2026-06 追加・TASK-0135）**: 上記の「GPU 依存の別作業」に加え、本 worktree で利用可能な本物の runtime 計測として CPU-proxy A/B が同じく橋渡しされた。`reproduction_record_from_ab_measurements(baseline_backward, reproduction_backwards)` が Phase 67 in-vivo suite の `_BackwardCounter`（各 Level-2 suffix 切断再生産の後方走査回数）を `ReproductionRecord` に変換し、`compare_freeze_levels(reproduction_record=)` が直消費する。配線した CPU-proxy A/B を depth {2,3,4} の 3 再生産で走らせると、3 つの実 headline（depth に応じて異なる・accountant の Level-2 予測と 1e-9 で一致）が N>=3 の非薄観測となり、点推定だったヘッドラインは実幅の bracket に、`format_level_comparison()` は `reproduction_bracket`/`calibrated`/`n=3` の監査行を出力する（record なしでは出さない = byte-identical）。9B スケールの CUDA A/B bracket は引き続き GPU 環境の別作業だが、CPU-proxy は偽造ではない実測源として今すぐこの着地点を厚くする。
