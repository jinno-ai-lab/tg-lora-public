...
- この full run の各層・各データの入力活性 `xin` を全サイクルでダンプ → 後続の検証の target ソースを兼ねる。
- baseline_plain best_valid=1.0565、accum16=1.0704 と整合確認。

#### Phase 1: 最小ゲート（単層フリーズ）
- full run が十分成熟した後の **最終段1層だけ** をフリーズ。
- その `xin` をキャッシュし、直前の1層だけを activation matching（まず素直に MSE）で学習。
- **判定基準**:
  - activation matching で学習した層の最終 valid_loss が、最終 loss で学習した版にどれだけ迫るか（代理ロスの整合性）
  - 後段1層を貫通しないことで backward がどれだけ減ったか（計算の実測）
...
#### Phase 4: 跨条件検証（スケジュールの移植性）
見つけた最適フリーズスケジュールを、異なる条件に適用して有効性を確認:
- 学習率を変える
- データを一部入れ替える/追加する
- LoRA rank を変える
- シードを変える
...
1. **Phase 0: full backprop run**（ベースライン + target 生成）
   - 全 LoRA 層を最終 loss で学習。valid_loss・総 backward コストを記録。
   - 各層の `xin` を全サイクルでダンプ（Phase 1 の target 用）。
   - まず最終段の数層に絞ってダンプ（Phase 1 専用で軽い）。
   - baseline_plain / accum16 と整合確認。

2. **Phase 1: 単層ゲート**
   - full run 成熟後の最終段1層をフリーズ、`xin` キャッシュ。
   - 直前の1層を MSE activation matching で学習。
   - **判定**: activation matching 版の valid_loss が最終 loss 版に迫るか。backward 削減の実測。
