...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate JGLUE tasks from llm-jp-eval"
    )
    parser.add_argument(
        "--model-path",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="Max number of examples per task to evaluate",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default="reports/llm_jp_eval_mlx",
    ...
    datasets_raw = {}
    try:
        datasets_raw["jcommonsenseqa"] = load_dataset(
            "sbintuitions/JCommonsenseQA", split="validation"
        )
    ...
    for task_name, ds in datasets_raw.items():
        ...
        for item in tqdm(examples, desc=f"Evaluating {task_name}"):
            ...
            if task_name == "jcommonsenseqa":
                # Check if generated contains correct label character
                is_correct = expected in gen_text[:5]
            elif task_name == "jnli":
                # Check for exact matches
                is_correct = expected in gen_text
            elif task_name == "jsquad":
                # SQuAD exact match or high F1
        ...
        logger.info(
            f"Task {task_name} complete: Accuracy={accuracy:.4%}, Mean Char-F1={mean_char_f1:.4f}"
        )
    ...
    report_data = {
        ...
        "results": {
            k: {"accuracy": v["accuracy"], "mean_char_f1": v["mean_char_f1"]}
    ...
    with open(report_md_path, "w", encoding="utf-8") as f:
        ...
        for k, v in results.items():
            f.write(
                f"| **{k.upper()}** | {v['accuracy']:.2%} | {v['mean_char_f1']:.4f} |\n"
            )
