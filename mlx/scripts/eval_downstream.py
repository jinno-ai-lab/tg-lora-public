...
from mlx_lm.utils import load as mlx_load  # noqa: E402
        ...
        summary.update(
            {
                ...
                "key_compliance_rate": key_compliance_count / total
                if total > 0
                else 0.0,
            }
        )

    return {"summary": summary, "details": eval_results}


def main():
    parser = argparse.ArgumentParser(
        description="Evaluate downstream tasks for MLX models/adapters"
    )
    parser.add_argument(
        "--model-path",
        type=str,
        default=".cache/mlx_models/Qwen--Qwen3.5-9B",
        help="Path to MLX model folder",
    )
    parser.add_argument(
        "--adapter-path", type=str, default=None, help="Path to MLX adapter folder"
    )
    parser.add_argument(
        "--max-examples",
        type=int,
        default=None,
        help="Limit number of examples evaluated",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default="reports/downstream_eval_mlx",
    ...
    if not jp_dataset_path.exists() or not json_dataset_path.exists():
        raise FileNotFoundError(
            "Downstream datasets must be pre-created under data/downstream/"
        )
    ...
    report_data = {
        ...
        "results": {"jp_capability": jp_results, "format_json": json_results},
    }
    ...
    with open(report_md_path, "w", encoding="utf-8") as f:
        ...
        f.write(
            f"- **Adapter Checkpoint**: `{report_data['adapter_path'] if report_data['adapter_path'] else 'None (Base Model)'}`\n\n"
        )
        ...
        f.write("| --- | --- | --- |\n")
        f.write(
            f"| **Japanese Capability** | Mean Char-F1 | {jp_results['summary']['mean_char_f1']:.4f} |\n"
        )
        f.write(
            f"| **JSON Format Compliance** | Mean Char-F1 | {json_results['summary']['mean_char_f1']:.4f} |\n"
        )
        f.write(
            f"| | JSON Validity Rate | {json_results['summary']['json_validity_rate']:.4%} |\n"
        )
        f.write(
            f"| | Key Compliance Rate | {json_results['summary']['key_compliance_rate']:.4%} |\n\n"
        )
