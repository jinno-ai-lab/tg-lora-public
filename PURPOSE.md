...
1. **MS-PF4 / 分類 A**: schedule-portability の単体テスト — 同一スケジュール手順が異なる
   active_layer 構成で破綻しないことの CPU 検証（GOAL §3.1 Phase 4 / §4 step 5）。
   MS-PF2 の Category-A は 3/3、MS-PF3 の Category-A も本イテレーションで **2/2 完全閉包**
   （local-loss per-arm breakdown 観測 + 損失アブレーションハーネス `loss_ablation.py`）。
2. 理由: Phase 2/3 の CPU 側足場は枯れた。残る Category-A は Phase 4 の schedule-portability
   検証のみ。これを閉じれば PF2/PF3/PF4 の Category-A が全て閉包し、未達は分類 C（GPU run）
   のみとなる — GPU 利用可能次第に valid_loss 実測が即座に回せる状態を維持する。
