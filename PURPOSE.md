...
- **分類 A 変換（コードで解決可能）**:
  ...
  - [ ] **frontier sweep の CLI exposition** — `freeze_frontier.frontier()` を bare CLI から起動できるようにし（`scripts/`）、Phase 2 計画を GPU run 前に再現可能に吐く。
...
1. **MS-PF2 / 分類 A**: frontier sweep の CLI exposition（`scripts/run_freeze_frontier.py` 相当）—
   Phase 2 計画を GPU run 前に再現可能に出力。GOAL §7「GPU run 前に機構を検証」の直接実践。
2. 理由: MS-PF2-CPU の予測面は完成しているが、bare CLI からの起導線が無く、Phase 2 計画の
   再現性が手作業に依存している。CLI 化は Category A・CPU 完結・GOAL §4/§7 に直接寄与する。
