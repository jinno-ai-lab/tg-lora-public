...
- **分類 A 変換（コードで解決可能）**:
  ...
  - [ ] **サロゲート超過の判定ヘルパ** — candidate schedule の削減/性能が random-order surrogate を超えるかを 1 関数で判定（valid_loss 軸は GPU 依存だが、構造と seed 管理は CPU で固定可能）。
...
1. **MS-PF2 / 分類 A**: サロゲート超過の判定ヘルパ — candidate schedule の削減/性能が
   `random_freeze_order` サロゲート（MS-PF2 surrogate null）を超えるかを 1 関数で判定。
   valid_loss 軸は GPU 依存だが、構造・seed 管理・FLOPs 軸の比較は CPU で固定可能。
   GOAL §4「ランダム順フリーズ対照を超えて初めて主張」の直接実践。
2. 理由: frontier sweep の CLI exposition（`scripts/run_freeze_frontier.py`）は本イテレーション
   で閉包。MS-PF2 の Category A で残る足場は「surrogate 超過」判定で、これが GOAL §4 の有効性
   ゲートを CPU 側で表現する最後の部品。構造と seed を CPU で固定すれば、GPU run 時に
   pass/fail が即座に確定する状態を維持できる。
