...
- **Category-A helpers 残数: 0**（MS-PF2 4/4・MS-PF3 2/2・MS-PF4 1/1 = 計 **7/7 完了**）:
  ...
  **本イテレーションは Cat-A ヘルパーを 0 件追加**（feedback の収益逓減指示に合致）—
  generalization task / heterogeneous stack / **order-sensitivity 診断** は Cat-A 足場ではなく
  **実 GPU run の解像力を上げる Cat-C 証拠拡張**（同じ実行導線・同じ trio・足場ヘルパーではない。
  診断は `surrogate_valid_loss_ci` を呼ばず verdict を出さない=測定科学的ステップ）。
...
1. **target-scale valid_loss 判定** — private `src.data` pipeline 利用可能次第、9B QLoRA run の
   ...
   order-sensitivity 診断（ratio=0.000）が proxy では順序が非解像と証明した以上、target-scale は
   順序効果を解像する**唯一の**手段（**必要と証明済み**）。
