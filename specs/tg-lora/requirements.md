...
- REQ-149: Makefileはbench-velocity-ops-ciターゲットを提供し、scripts/benchmark_velocity_ops.pyを`--quick --max-cap-overhead-ratio 3.0`（同一run内のcapped/no-cap per-iter ms 比 = host速度が相殺されたhardware-normalized overhead不変量）で実行し、ratioがceilingを超える算法的回帰（冗長clone/norm・out-of-place化等）を自動検出しなければならない。検出時はexit code 1でCI失敗としなければならない。**絶対時間の`--baseline baselines/velocity_ops.json --threshold`比較はhost速度に依存して非portable**（12GB box・pytest負荷下で12x false regression を生じgate常時RED化）のためCI gateからは除外し、`bench-velocity-ops-save-baseline`と共にlocal same-host diagnosticとして残存させる。baselines/velocity_ops.jsonはリポジトリにチェックインされ続ける。design-interview.md A31で🔴（未設計）として指摘されていたCI回帰閾値の設計ギャップを閉じる 🔵 *2026-07-05 portable化（`--max-cap-overhead-ratio`追加・Makefile + .github/workflows/test.yml gates job + tests/test_ci_gate_wiring.py でpin・本REQ-149仕様も新commandに更新）。origin: benchmark_velocity_ops.py --baseline/--threshold実装・design-interview.md A31指摘・AI_HUB_MAKE_RUN_FEEDBACK「Wire bench-velocity-ops into CI」*
