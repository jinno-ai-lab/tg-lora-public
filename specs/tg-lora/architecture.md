...
- [freeze-ci-operator-errors 受け入れ基準](../freeze-ci-operator-errors/acceptance-criteria.md)
- [freeze-ci-operator-errors アーキテクチャ設計](../freeze-ci-operator-errors/architecture.md)
- [freeze-ci-operator-errors データフロー図](../freeze-ci-operator-errors/dataflow.md)
- [freeze-ci-operator-errors 設計自動分析記録](../freeze-ci-operator-errors/design-interview.md)
- [freeze-ci-operator-errors 自動分析記録](../freeze-ci-operator-errors/interview-record.md)
- [freeze-ci-operator-errors 要件定義書](../freeze-ci-operator-errors/requirements.md)
- [freeze-ci-operator-errors タスク概要](../freeze-ci-operator-errors/tasks/overview.md)
- [oper-decision-surface 自動分析記録](../oper-decision-surface/interview-record.md)
- [oper-decision-surface 要件定義書](../oper-decision-surface/requirements.md)
...
- [TASK-0130: Progressive Freezing schedule+frontier+local-loss E2E integration test](tasks/TASK-0130.md)
- [TASK-0131: ProgressiveFreezeController 多層プログレッシブ凍結（design §4.1）](tasks/TASK-0131.md)
- [TASK-0132: §6.2 Level-1 証拠配管 — 計測された in-vivo 実現削減が ceiling を引き上げて回復する着地点（design §6.2）](tasks/TASK-0132.md)
- [TASK-0133: §6.3 比較ヘッドライン再生産 bracket 着地点 — A/B 再生産観測が additional_realized_reduction の bracket を較正する着地点（design §6.3 / Phase 3）](tasks/TASK-0133.md)
- [TASK-0134: §6.3 分散較正バンドの非負床注記 — `format_reduction_band` が負の下限を達成可能と読ませない着地点（design §6.3）](tasks/TASK-0134.md)
- [TASK-0135: §6.2/§6.3 証拠配管を CPU-proxy in-vivo A/B 実測へ配線 — 薄い証拠の着地点を本物の観測で厚くし、判定経路を観察可能にする（design §6.2/§6.3 → Phase 3 A/B）](tasks/TASK-0135.md)
- [TASK-0136: 起点発射性の repo-wide schema gate — 任意の LoggingConfig/スキーマ変更が出荷 config を壊すのを、feature コミットと同一コミットで捕える（d2218ed 回帰クラスの一般化）](tasks/TASK-0136.md)
- [TASK-0137: M10.3 disk-death guard の第2ベクトル閉包 — `trajectory_delta_artifacts/*.pt` のサイクル毎無限蓄積を、既存の keep_last_checkpoints/min_free_disk_gb knob で抑える](tasks/TASK-0137.md)
- [TASK-0138: M10.3 disk-death guard の第3ベクトル閉包 — ベースライン学習エントリポイント（`checkpoint-<step>` + `trajectory_delta_artifacts/*.pt`）を既存 knob 傘下へ](tasks/TASK-0138.md)
- [TASK-0139: Guard 実験 §4 可逆解放の不活性化欠陥閉包 — `DynamicFreezeController` の攪拌/上流活性リリースが同サイクルで即時再凍結される（リリースが no-op）+ §3/§4 制御カバレッジ](tasks/TASK-0139.md)
- [TASK-0140: Guard 実験 §5.2 品質ゲートの品質信号誤参照閉包 — 決定関数が pilot/split 境界 proxy 損失（`loss_valid`）を設計が要求する full-eval 損失（`valid_full`）の代わりに読んでいた + §5.1/§5.2/§5.3 決定関数の直接カバレッジ 0](tasks/TASK-0140.md)
- [TASK-0141: Guard §5.2 誠実性契約の最終一哩 — trainer が `loss_valid_full` を step record に永続化して受口を活性化させる](tasks/TASK-0141.md)
- [TASK-0142: 論文 Claim Gate G1/G4 の既知制限を正式記録 — gate 評価器に known-limitation を発火させ、具体 next action + owner で追跡可能にする](tasks/TASK-0142.md)
- [TASK-0143: §5.2 誠実性契約の休眠を可視化 — analyzer が pilot proxy に黙示フォールバックしたとき DORMANT 警告を発し TASK-0141 への所有を付与する](tasks/TASK-0143.md)
- [TASK-0144: 論文 Gate 評価器に INSUFFICIENT EVIDENCE 第3状態を導入 — 入力欠落 bail を FAIL（反証）と区別し、generic owner の G1/G4 task 誤帰属を修正](tasks/TASK-0144.md)
- [TASK-0145: consolidate_paper_results に INSUFFICIENT 第3状態を伝播 — 未測定 gate が反証 claim のように読めるのを claim-level 報告で阻止](tasks/TASK-0145.md)
- [TASK-0146: 論文 Gate 評価器 CLI の発射性閉包 — `evaluate_paper_gates.py` が単体 CLI として起動できず、自身の honesty-contract E2E 検証（TASK-0144/0145）を死状態にしていた欠陥](tasks/TASK-0146.md)
- [TASK-0147: 残りの bare-CLI 発射不能スクリプト群の sys.path bootstrap 閉包 — `evaluate_paper_gates.py`（TASK-0146）と同一の「E2E test が import 死で不活性」欠陥が6 script に残存していたのを閉じる](tasks/TASK-0147.md)
- [TASK-0148: 最後の bare-CLI 発射不能スクリプト群の sys.path bootstrap 閉包 — TASK-0146/0147 と同一の「CLI test が import 死で不活性」欠陥が bootstrap-fixable な script に残存していたのを閉じる（7 script 緑化 + 1 script bootstrap 適用・dep-blocked 明示）](tasks/TASK-0148.md)
- [TASK-0149: G2 frontier-separation gate の INSUFFICIENT-EVIDENCE 契約閉包 — `_check_g2` が frontier report（G2 の required input）欠損/破損時に `evaluated=False` で bail せず disproven FAIL になっていた（G3/G4 は既に同一契約を履行中）](tasks/TASK-0149.md)
- [TASK-0150: §5.2 誠実性契約の guard(B) 側閉包 — A/B 比較の休眠可視化が baseline(A) 側のみで、dormant な guard run が time-to-quality 比較を黙示汚染していた](tasks/TASK-0150.md)
- [TASK-0151: spine-anchor 整合性の自動検査導入 — `specs/`+`docs/` の 158 個の spine-anchor ブロックが「markdown link と `parent:` 論理パスの二重記述」で親文書を再宣言していたが、両者の不一致（re-point 片側更新）やリネーム/削除による drift を捕捉する検査が一つもなく、手動昇格レビューで case-by-case に見つけていた欠陥を自動化で閉じる](tasks/TASK-0151.md)
...
- [TASK-0218: feedback bullet 3 の全系監査で ledger resume-fingerprint の model-quant 欠落を発見→閉鎖（silent-replay gap・corrupt-but-green §4 class）](tasks/TASK-0218.md)
